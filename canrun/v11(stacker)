...
void* pthreadMultiplication(void* arg) {
    ...
    int startRow = args->i;
    int endRow = args->j;
    //printf("Execution: Thread  %c = %c %c %c    row: %d - %d\n",result->name, matrix1->name,'*',matrix2->name, startRow, endRow);
...
//    start = clock(); // Start the timer
    ...
    // Create and execute three threads
      for (int i = 1; i < subPthreadNum; i++) {
          ...
          //printf("Execution: Thread  %c = %c %c %c    row: %d - %d\n",result->name, matrix1->name,operatorCh,matrix2->name, subArgs[subPthreadCount].i, subArgs[subPthreadCount].j);
...
int Operation_logic(char expression[50], Matrix* matrices)
{
    //For pthread
    int threadcount;
    int subPthreadSize = NUM_THREADS;
    int rc;
